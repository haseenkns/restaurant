 ...
 * @version 	1.1 (PHP5)
 * http://www.gen-x-design.com/demos/thumbnailer/   for demo...
 * http://www.gen-x-design.com/projects/php-thumbnailer-class/  for full details.....
 */
...
//echo $this->format;
    ...
    public function rotateImage($direction = 'CW') {
		...
		public function showThumb($image,$w,$h,$thumb){
		echo "$image";
		exit;
			//$thumb->resize($w,$h);
			//$thumb->save('../images/thumb/'.$image,100);
			//return  '<img src="../images/thumb/"'.$image.' border="0">';
  ...
  function showimageThumb($imagepath,$w,$h){
		
		if(file_exists($imagepath)){
		 $imageName=$w."_".basename($imagepath);
		  if(file_exists("thumb/".$imageName)){
		 echo "<img style='border:solid 2px #CCCCCC;padding:2px;' src= 'thumb/".$imageName."' border='0'>"; 
		 }else{
		$imageObj=new Thumbnail($imagepath);
		$imageObj->resize($w,$h);
		$imageObj->save("../thumb/".$imageName,$w);
		echo "<img style='border:solid 2px #CCCCCC;padding:2px;' src= '../thumb/".$imageName."' border='0'>";   
		  
		}}else{
		echo "<img style='border:solid 2px #CCCCCC;padding:2px;' src= '../images/nophoto.gif' width='".$w."' height='".$h."' border='0'>";  
		}
  }
   function showFrontimageThumb($imagepath,$w,$h){
		if(file_exists($imagepath)){
		//echo $imagepath;
		 $imageName=$w."_".basename($imagepath);
		 if(file_exists("thumb/".$imageName)){
		 return   "<img  src= 'thumb/".$imageName."' border='0' class='imgborder'>"; 
		 }else{
		$imageObj=new Thumbnail($imagepath);
		$imageObj->resize($w,$h);
		$imageObj->save("thumb/".$imageName,$w,$h);
		 return   "<img  src= 'thumb/".$imageName."' border='0' class='imgborder'>"; 
		}}else{
		return "<img  src= 'images/noimage.jpg' width='".$w."' height='230px' border='0' class='imgborder'>";  
		}
  }
   function showLightboxThumb($imagepath,$w,$h){
		if(file_exists($imagepath)){
		//echo $imagepath;
		 $imageName=$w."_".basename($imagepath);
		 if(file_exists("thumb/".$imageName)){
		...
		return "<img  src= 'images/noimage.jpg' width='".$w."' height='".$h."' border='0' class='imgborder'>";  
		}
  }
  
  function showFrontimageThumbSrc($imagepath,$w,$h){
		if(file_exists($imagepath)){
  ...
  function showVideoThumb($imagepath,$w,$h){
		if(file_exists($imagepath)){
		 $imageName=$w."_".basename($imagepath);
		 if(file_exists("thumb/".$imageName)){
		 echo "thumb/".$imageName; 
		 }else{
		$imageObj=new Thumbnail($imagepath);
		$imageObj->resize($w,$h);
		$imageObj->save("thumb/".$imageName,$w);
