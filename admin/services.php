...
if(isset($_GET['did'])&&$_GET['did']!=''){
	...
	$selimgQry=mysql_fetch_row(mysql_query("select * from  `services` where `id`='$delid'"));
	$oldimg=$selimgQry[1];

	$unlinkimg=unlink("../photos/".$oldimg);
	
		$delQry=mysql_query("delete from `services` where id='$delid'");
		if($delQry){
			$successflag=1;
		}else{
			$successflag=0;
	...
	if($successflag=1){
		header("location:services.php?msg=dls");
	}else{
		header("location:services.php?msg=dlf");
	} 
 }
...
if(isset($_POST['submit'])|| isset($_POST['submit_x'])){
	...
	$services = $_POST['operational'];
	if(count($services)>0){
		$operational=implode(",",$services);	
	}else{
		$operational=0;
	}
	...
	$pid=$hidAid;
	if($moveimg){
	$sqlQry="insert into `services` set `imagepath`='$imgName' , `status`='1',`title`='$title',`position`='$position',`content`='$content',`pdate`='$pdate',`ptype`='$ptype' ,`operational`='$operational' ";
	...
	if($successflag==1){
		header("Location:services.php?msg=ins");
	}
	if($successflag==0){
		header("Location:services.php?msg=inf");
	}
...
$services = $_POST['operational'];
	if(count($services)>0){
 	 ...
 	 $title=mysql_real_escape_string($title);
			...
			$resImgQry=mysql_fetch_row(mysql_query("select `imagepath` from `services` where `id` = '$id'"));
			$oldimg=$resImgQry[0];
			...
			if($img!=''){
			 ...
			 $moveimg=move_uploaded_file($imagenamesrc,'../photos/'.$imgName);
					if($moveimg){
						...
						$sqlImgQry=mysql_query("Update `services` set `imagepath`='$imgName' ,`title`='$title',`position`='$position',`content`='$content'  ,`operational`='$operational' where `id`='$id'");
					}else{
						$successflag=0;
					}
			}else{
			  $sqlImgQry=mysql_query("Update `services` set `title`='$title' ,`position`='$position',`content`='$content' ,`operational`='$operational'   where `id`='$id'");
			  if($sqlImgQry){
			   $successflag=1;
			  }else{
			   $successflag=0;
	...
	if($successflag==1){
	    header("Location:services.php?msg=ups");
	}else{
        header("Location:services.php?msg=upf");
	}
...
<body class="theme-dark">
	...
	<div id="container">
		...
		<div id="content">
             ...
             <?php } ?>
              ...
              <tr>
				...
				<?php if(isset($_GET['eid'])&&$_GET['eid']!=''){ 
					...
					$execQry=mysql_fetch_row(mysql_query("select * from `services` where `id`='$eid'"));
              ...
              <td colspan="2" align="left">
				...
				<tr >
				<td>&nbsp;</td>
				<td colspan="2" align="left"><input type="hidden" name="hidId" value="<?php echo $eid; ?>"><input type="hidden" name="updateId" id="updateId"><input type="submit" name="update" class="btn" value=" Update Gallery">&nbsp;<input type="button" name="cancel" value=" Cancel " onClick="javascript:window.location.href='services.php'" class="btn"></td>
				</tr>
            ...
            <tr >
              ...
              <td colspan="2" align="left">
				...
				<?php
                    ...
                    if($numRows>0){
                    while($fetch=mysql_fetch_array($execQry)){?>
                           <div style="float:left;width:180px;padding:5px;border:dotted 1px #CCC;margin:5px;"><input type="checkbox" name="operational[]"   value="<?php echo $fetch['city_id'] ?>"> &nbsp; <b><?php echo $fetch['city_name']; ?></b></div>
                    <?php }
                    }else{?>
                           <div style="float:left;width:200px;">No Operational Areas</div>
                    <?php }?>
            ...
            <tr >
            <td>&nbsp;</td>
            <td colspan="2" align="left"><input type="hidden" name="hidCid" value="<?php echo $cid; ?>"><input type="submit" name="submit" class="btn" value=" Add  Service  ">&nbsp;<input type="button" name="cancel" value=" Cancel " onClick="javascript:window.location.href='services.php'" class="btn"></td>
            </tr>
				<?php } ?>
  				</form>
</table>
...
$j(document).ready(function(){ 
	...
	$j(function() {
		$("tbody").sortable({ opacity: 0.6, cursor: 'move', update: function() {
			var order = $(this).sortable("serialize") + '&action=updateRecordsListings&type=services'; 
				...
				<table width="100%" border="0" cellpadding="4" cellspacing="1" class="table table-striped table-bordered table-hover  datatable">
                 <thead>
                    <tr >
                        ...
                        <th align="left"  width="16%"class="verysmalltextblack">Name</th>
                        <th align="left"  width="36%"class="verysmalltextblack">Operational Areas</th>
                        <th align="center"  width="11%"class="verysmalltextblack">Add Sub Services</th>
  ...
  <?php 
  	$sqlQry=mysql_query("select * from `services`  order by `position` Asc, `id` desc");
	$i=0;
  ...
  <tr bgcolor="#FFFFFF" id="recordsArray_<?php echo $fetch['id']; ?>" onmouseover="this.style.cursor='move'">
    ...
    <td align="left" class="smallfonttext"><?php echo getOperationalArea($fetch['operational']); ?></td>
    <td align="center" class="smallfonttext"><a href="subservices.php?id=<?php echo $fetch['id']; ?>"><img src="../images/addsubservices.png"></a></td>
    <td align="center" >
      <table    border="0" cellpadding="0" cellspacing="0" style="padding:0px">
        <tr>
          <td align="right" ><a href="services.php?did=<?php echo base64_encode($fetch['id']) ?>"  class="confirm-dialog" onClick="return confirm(' Are you sure you want to delete !!')"><button class="btn"><i class="icon-trash"></i></button></a></td>
          <td align="center" ><a href="services.php?eid=<?php echo base64_encode($fetch['id']) ?>"><button class="btn"><i class="icon-edit"></i></button>	</a></td>
