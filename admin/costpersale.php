...
if(isset($_GET['empid'])&&$_GET['empid']!=''){
	...
	//$reportto[]=$empid;
	$reportto=getReportToemployee($empid);
	array_unshift($reportto,$empid);
	//print_r($reportto);
	//die;
}
...
<body class="theme-dark">
	...
	<div id="container">
		...
		<div id="content">
            ...
            <th  data-hide="phone,tablet">Designation</th>
             <th  data-hide="phone,tablet">No Of Sale</th>
              <th  data-hide="phone,tablet">Amount Paid</th>
            <th  data-hide="phone,tablet" style="text-align:center">Cost Per sale</th>
  </tr>
									...
									<tbody>
										<?php 
	$impId=implode(",",$reportto);							
  	$sqlQry=mysql_query("select * from `admin`  where `id` IN ($impId)");
	$i=0;
	...
	$i++;
	$empid=$fetch['id'];
	$cost=getNumberofCostPerSalesByStartDayEndDay($pg,$month,$year,$empid);
	$paid=getEmployeeAmtPaidAsSalaryAndIncentive($empid,$month,$year);
	if(!$cost==0){
	$sum=floor($paid/$cost);
	}else{
		$sum=0;
  ...
  <tr bgcolor="#FFFFFF">
    ...
    <td align="left"  ><?php echo getTabledataById("name",'designations',$fetch['designation']); ?></td>
    <td align="center" class="smallfonttext"><?php echo $cost ?></td>
    <td align="center" class="smallfonttext"><?php echo $paid ?></td>
    <td align="center" bgcolor="#F9F9F9"><?php echo $sum ?></td>
  </tr>
  <?php }}else{?>
  <tr><td>No Record</td><td>No Record</td><td>No Record</td><td>No Record</td><td>No Record</td><td colspan="1" align="center" class="blackbold" bgcolor="#FFFFFF">No Records Found !</td><td>No Record</td><td>No Record</td></tr>
