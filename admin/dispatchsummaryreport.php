...
if(isset($_GET['start']) && $_GET['start']!='' && $_GET['end'] && $_GET['end']!=''){ 
	...
	//die;
	$qrysText="pg=$pg&start=$startdate&end=$enddate";
}else{
...
<head>
    ...
    function seachEmbByDate(start,end,pg){
 	   window.location.href="dispatchsummaryreport.php?start="+start+"&end="+end+'&pg='+pg;	
    }
	
		function sortByProg(val){
		if(val==0){
			window.location.href="dispatchsummaryreport.php";
	    }else{
			window.location.href="dispatchsummaryreport.php?pg="+val;	
		}
	}
    </script>
	</head>
...
<body class="theme-dark">
							...
							<div class="widget-content no-padding">
								<table  class="table table-striped table-bordered table-hover table-checkable table-responsive  datatable">
									<thead>
                                         <tr >
                                            <th width="3%" >Sno</th>
                                            <th width="8%"  data-hide="phone" style="text-align:center">Number Of Dispatches</th>
                                            <th width="8%" data-hide="phone,tablet" style="text-align:left;">StateWise</th>
	...
	<?php 
	//echo $qrysText;
  	$sqlQry=mysql_query("select * from `state` where `status`='1' order by `id` Asc");
	$i=0;
	$numrows=mysql_num_rows($sqlQry);
	if($numrows>0){
	while($fetch=mysql_fetch_array($sqlQry)){
		$i++;
	$total=getTotalDispatchByStaeAndDate($fetch['id'],$startdate,$enddate,$pg);
		
			
  ?>
 		   <tr bgcolor="#FFFFFF">
            <td align="center" class="smalltext" <?php if($total>=1){ ?> style="background-color:#093;color:#FFF;" <?php } ?>><?php echo $i; ?></td>
            <td align="left" class="smallfonttext" style="text-align:center;" ><b><?php echo getTotalDispatchByStaeAndDate($fetch['id'],$startdate,$enddate,$pg); ?></b></td>
            <td align="left" class="smallfonttext"><a href="dispatchsummaryreportcitywise.php?sid=<?php echo $fetch['id']; ?>&<?php echo $qrysText; ?>"><?php echo $fetch['name']; ?></a></td>
           
  </tr>
  <?php }}else{?>
  <tr style="background:#FFF;"><td></td><td></td><td></td></tr>
