...
if(isset($_POST['submit'])){
		...
		if($flag==1){
		$excQry=mysql_query("INSERT INTO `members` (`id`, `prog_id`, `title`, `fname`, `mname`, `lname`, `gender`, `email`, `pcontact`, `scontact`, `dob`, `marital`, `doa`, `address1`, `address2`, `state`, `city`, `cityother`, `pincode`, `source`, `imagepath`, `amount`, `modeofpayment`, `pickup`, `employee`, `tenure`, `createdon`, `status` ,`mlevel`,`dateofsale`,`a_id`,`phoneres`,`phoneoff`,`spname`,`spemail`,`spmobile`,`spdob`,`remark`,`nameoncard`,`designation`,`compname`,`spousecard`,`voucherno`,`addresstype`,`creditedon`,`complimentry`,`approvedby`,`processedby`,`createdtime`,`renewed`) VALUES (NULL, '$program', '$title', '$fname', '$mname', '$lname', '$gender', '$email', '$pcontact', '$scontact', '$dob', '$marital', '$doa', '$address1', '$address2', '$state', '$hidCity', '$city', '$pincode', '$source', '$imgName', '$amount', '$modeofpayment', '$pickup', '$employee', '$tenure', '$pdate', '1','$hidMlevel','$dateofsale','$consultants','$phoneres','$phoneoff','$spname','$spemail','$spmobile','$spdob','$remark','$nameoncard','$designation','$compname','$spcard','$voucherno','$addresstype','$creditedon','$complimentry','$approvedby','$processedby','$ptime','1');");
		
	if($excQry){
	  $upsQry=mysql_query("Update `members` set `status` = '3' where `id` ='$rid'");	
		$flag=1;
		if(!$complimentry==1){	 
	...
	if($flag==1 ){
		mysql_query("COMMIT");
		header("location:thanksrenewal.php?msg=ins");					  
	}else{
		mysql_query("REVOKE");
		header("location:thanksrenewal.php?msg=inf");	
	}
                                    ...
                                    <div class="form-group">
                                      ...
                                      <div class="form-group">
                                            ...
                                            <div class="col-md-2">
                                                ...
                                                <div ><img src="photos/<?php echo $userData[20]; ?>" style="border-radius:20px;"><input type="hidden" name="hidImage" value="hidImage" value="<?php echo $userData[20];  ?>"></div>
												</div>
					...
					<div class="col-md-12">
						<div class="widget box">
                            ...
                            <hr/>
                                    ...
                                    <label class="radio-inline">
                                    <input type="radio"   class="uniform" name="pickup" id="pickup" value="2"  checked>
                                    Or Transferred
                                    </label>
                                    </div>
                                    </div>
                                    ...
                                    <input type="text" name="creditedon" id="creditedon" value="" class="form-control input-width-large" data-mask="99/99/9999">
                                    <span class="help-block">DD/MM/YYYY</span>
                            ...
                            <hr/>
                                    ...
                                    <div class="col-md-8">
                                   <select name="approvedby" id="approvedby" class="form-control "  ><option value="0">Select Consultant</option>
				<?php
					$execQry=mysql_query("select * from `admin` where `status` = '1' and `skey`!='1' ");
					$numRows=mysql_num_rows($execQry);
					if($numRows>0){
					...
					<?php } ?>
                                    ...
                                    <div class="col-md-8">
                                    <select name="processedby" id="processedby" class="form-control "  ><option value="0">Select Consultant</option>
				<?php
					$execQry=mysql_query("select * from `admin` where `status` = '1' and `skey`!='1' ");
					$numRows=mysql_num_rows($execQry);
					if($numRows>0){
					...
					<?php } ?>
                                    ...
                                    <div class="row" style="padding-top:30px;">
										...
										<div class="col-md-12">
                                            ...
                                            <div class="col-md-3" align="left">
                                           <input type="hidden" name="hidId" value="<?php echo $eid; ?>"><input type="submit" name="submit" class="btn btn-primary btn-block" value=" Renew Member"> 
