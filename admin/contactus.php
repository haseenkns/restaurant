...
if(isset($_GET['msg'])&&$_GET['msg']!=''){	
	...
	$select1=mysql_num_rows(mysql_query("select * from cms where  `key`='3'"));
if(isset($_POST['submit']))
		{
			//$select=mysql_num_rows(mysql_query("select * from cms where  `key`='2'"));
		   if($select1<1)
		   {    echo 
		  	    $insert=mysql_query("insert into cms set `id`='',`content`='".$_POST["content"]."',`title`='".mysql_real_escape_string($_POST["heading"])."' ,`key`='3' ");
		   		if($insert){
					header("location:contactus.php?msg=ins");
				}else{
					header("location:contactus.php?msg=inf");
				}
		   }
		   else
		   {
			   $update=mysql_query("update cms  set content='".$_POST["content"]."',`title`='".mysql_real_escape_string($_POST["heading"])."'  where `key`='3'");
			   if($update){
			   		header("location:contactus.php?msg=ups");
			   }else{
			   		header("location:contactus.php?msg=upf");
			   }
			}
		}

	$myselect=mysql_query("select * from cms where  `key`='3'");
					...
					<div class="col-md-12">
						<div class="widget box">
							<div class="widget-header">
								<h4><i class="icon-reorder"></i> Contact Us Content</h4>
							</div>
