...
if(isset($_GET['did'])&&$_GET['did']!=''){
	...
	$delQry=mysql_query("delete from `referencemembers` where `id`='$did'");
		if($delQry){
			header("location:referencemembers.php?msg=dlsaid=".base64_encode($memId)."");
		}else{
			header("location:referencemembers.php?msg=dlfaid=".base64_encode($memId)."");
		}
}
...
if(isset($_POST['submit'])){
		...
		$excQry=mysql_query("INSERT INTO `referencemembers` (`id`, `mem_id`, `title`, `name`, `email`, `contact`,`createdon`, `status`) VALUES (NULL, '$hidMemId','$title', '$name', '$email', '$contact','$pdate', '1');");
	...
	if($flag==1 ){
		header("location:referencemembers.php?msg=ins&aid=".base64_encode($hidMemId)."");					  
	}else{
		header("location:referencemembers.php?msg=inf&aid=".base64_encode($hidMemId)."");	
	}
 }
...
if(isset($_POST['update'])){
		...
		$email=mysql_real_escape_string($_POST['email']);
		 ...
		 $sqlQry="UPDATE `referencemembers` SET  `title` = '$title', `name` = '$name', `email` = '$email', `contact` = '$contact'  WHERE `referencemembers`.`id` = '$id';";
		//die;
...
if($flag==1 ){
		header("location:referencemembers.php?msg=ups&aid=".base64_encode($hidMemId)."");					  
	}else{
		header("location:referencemembers.php?msg=upf&aid=".base64_encode($hidMemId)."");	
	}
...
if(isset($_GET['eid'])&&$_GET['eid']!=''){
	...
	$edtQry=mysql_query("Select * from `referencemembers` where `id`='$eid'");
			...
			<div class="container">
				...
				<div class="page-header">
					<div class="page-title">
						<h3>Referrenced By - <?php echo $memName; ?>  </h3>
...
<form action="" method="post" enctype="multipart/form-data" onSubmit="return ValidateMember()">
 <?php if($msg!=''){ ?>
   ...
   <?php 
    if(isset($_GET['eid'])&&$_GET['eid']!=''){?>
                                            ...
                                            <div class="col-md-2">
												...
												<div class="col-md-3">
                                                 <label class="control-label">Name *</label>
													<input type="text" name="name" id="name" class="form-control" value="<?php echo  htmlentities(stripslashes($userData[3])) ?>">
												</div>
												<div class="col-md-3">
                                                ...
                                                <label class="control-label">Contact</label>
													<input type="text" name="contact" id="contact" class="form-control" value="<?php echo htmlentities(stripslashes($userData[5])) ?>">
												</div>
											</div>
     ...
     <?php } ?>       
        ...
        <div class="row">
            ...
            <th width="25%" data-hide="phone,tablet">Contact</th>
           
            <th width="15%"  data-hide="phone,tablet">Email</th>
	...
	<tbody>
	<?php 
	$sqlQry=mysql_query("select * from `referencemembers` where `mem_id`='$memId' order by `id` Desc");
	$i=0;
	$numrows=mysql_num_rows($sqlQry);
	if($numrows>0){
	while($fetch=mysql_fetch_array($sqlQry)){
	$i++;
	$name=getTabledataById("name","titles",$fetch['title'])." ".$fetch['name'];
   ?>
  <tr bgcolor="#FFFFFF">
       <td align="center" class="smalltext"><?php echo $i; ?></td>
       <td align="center"  ><a href="viewreferencemember.php?aid=<?php echo base64_encode( $fetch['id']); ?>"  > <button type="button" class="btn"><i class="icon-desktop"></i></button></a></td>
	
    <td align="left" class="smallfonttext"><?php echo $name; ?></td>
	<td align="left" class="smallfonttext"><?php echo $fetch['email'] ?></td>
 
	<td align="center"  ><?php echo $fetch['contact']; ?></td>
    
    
    <td align="center" bgcolor="#F9F9F9"><table    border="0" cellpadding="0" cellspacing="0" style="padding:0px">
	<tr>
	<td align="right" ><a href="referencemembers.php?did=<?php echo base64_encode($fetch['id']) ?>&aid=<?php echo base64_encode($memId); ?>"  class="confirm-dialog" onClick="return confirm(' Are you sure you want to delete !!')"><button class="btn" type="button"><i class="icon-trash"></i></button></a></td>
		<td align="center" ><a href="referencemembers.php?eid=<?php echo base64_encode($fetch['id']) ?>&aid=<?php echo base64_encode($memId); ?>"><button class="btn" type="button"><i class="icon-edit"></i></button>	</a></td>
        
        
		<td align="left"  ><input class='uniform' type="checkbox" id="check<?php echo $fetch['id']  ?>" value="<?php echo $fetch['status']  ?>" onClick="updateStatus('<?php echo $fetch['id'];  ?>','referencemembers',7)" <?php if($fetch['status']==1){echo 'checked';} ?>></td>
