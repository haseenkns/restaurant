...
if(isset($_GET['msg'])&&$_GET['msg']!=''){	
	...
	$select1=mysql_num_rows(mysql_query("select * from cms where  `key`='8'"));
if(isset($_POST['submit']))
		{
			//$select=mysql_num_rows(mysql_query("select * from cms where  `key`='2'"));
		   if($select1<1)
		   {    echo 
		  	    $insert=mysql_query("insert into cms set `id`='',`content`='".$_POST["content"]."',`title`='".mysql_real_escape_string($_POST["heading"])."' ,`key`='8' ");
		   		if($insert){
					header("location:term.php?msg=ins");
				}else{
					header("location:term.php?msg=inf");
				}
		   }
		   else
		   {
			   $update=mysql_query("update cms  set content='".$_POST["content"]."',`title`='".mysql_real_escape_string($_POST["heading"])."'  where `key`='8'");
			   if($update){
			   		header("location:term.php?msg=ups");
			   }else{
			   		header("location:term.php?msg=upf");
			   }
			}
		}

	$myselect=mysql_query("select * from cms where  `key`='8'");
					...
					<div class="col-md-12">
						<div class="widget box">
							<div class="widget-header">
								<h4><i class="icon-reorder"></i> Hr Policies Content</h4>
							</div>
