...
$delQry=mysql_query("delete from `titles` where `id`='$did'");
	if($delQry){
		header("location:titles.php?msg=dls");
	}else{
		header("location:titles.php?msg=dlf");
	}
}
...
$sqlQry="insert into `titles` set `name`='$name',`status`='1'";
$execQry=mysql_query($sqlQry);
if($execQry){
	header("location:titles.php?msg=ins");
}else{
	header("location:titles.php?msg=inf");
}
...
$name=mysql_real_escape_string($_POST['name']);
$id=$_POST['hidval'];
$sqlQry="Update `titles` set `name`='$name' where `id`='$id' ";
$execQry=mysql_query($sqlQry);
if($execQry){
	header("location:titles.php?msg=ups");
}else{
	header("location:titles.php?msg=upf");
}
...
<body class="theme-dark">
					...
					<div class="col-md-12">
						<div class="widget box">
							<div class="widget-header">
								<h4><i class="icon-reorder"></i> Add titles</h4>
							</div>
             ...
             <?php } ?>
              ...
              <tr>
				...
				<?php if(isset($_GET['eid'])&&$_GET['eid']!=''){ 
					...
					$execQry=mysql_fetch_row(mysql_query("select * from `titles` where `id`='$eid'"));
					$name=$execQry[1];
				...
				<tr >
				<td width="17%" align="left" class="blackbold">Update  titles Name</td>
				...
				<tr >
				<td>&nbsp;</td>
				<td colspan="2" align="left"><input type="submit" name="update" class="btn" value=" Update titles	 ">&nbsp;<input type="button" name="cancel" value=" Cancel " onClick="javascript:window.location.href='titles.php'" class="btn"></td>
				</tr>
				
				<?php }else{?>
				<tr >
				<td align="left" class="blackbold">Add titles  Name</td>
				...
				<tr >
				<td>&nbsp;</td>
				<td colspan="2" align="left"><input type="submit" name="submit" class="btn" value=" Add titles  ">&nbsp;<input type="button" name="cancel" value=" Cancel " onClick="javascript:window.location.href='titles.php'" class="btn"></td>
				</tr>
				<?php } ?>
  				</form>
</table>
			  ...
			  <tr><td colspan="2">
                ...
                <div class="row">
					<div class="col-md-12">
						<div class="widget box">
							<div class="widget-header">
								<h4><i class="icon-reorder"></i> titles</h4>
  ...
  <?php 
  	$sqlQry=mysql_query("select * from `titles` order by `id`");
	$i=0;
  ...
  <tr bgcolor="#FFFFFF">
	...
	<tr >
		<td align="right" ><a href="titles.php?did=<?php echo base64_encode($fetch['id']) ?>"  class="confirm-dialog" onClick="return confirm(' Are you sure you want to delete !!')"><button class="btn"><i class="icon-trash"></i></button></a></td>
		<td align="center" ><a href="titles.php?eid=<?php echo base64_encode($fetch['id']) ?>"><button class="btn"><i class="icon-edit"></i></button>	</a></td>
        
        
		<td align="left"  ><input class='uniform' type="checkbox" id="check<?php echo $fetch['id']  ?>" value="<?php echo $fetch['status']  ?>" onClick="updateStatus('<?php echo $fetch['id'];  ?>','titles',2)" <?php if($fetch['status']==1){echo 'checked';} ?>></td>
