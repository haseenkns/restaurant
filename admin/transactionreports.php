...
if(isset($_GET['msg'])&&$_GET['msg']!=''){	
	...
	if(isset($_GET['start']) && isset($_GET['end']) && isset($_GET['pg']) ){
		...
		$pg =$_GET['pg'];
		$type =$_GET['type'];
		$getTableMemIds=searchMembersByTransactions($pg,$start,$end,$type);
	}else{
		$getTableMemIds=allMembers()	;
	}
...
<head>
    ...
    <script>
		function seachByDate(start,end,program,type){
				...
				window.location.href="transactionreports.php?start="+start+"&end="+end+'&pg='+program+'&type='+type;	
	...
	<div id="container">
		...
		<div id="content">
			<div class="container">
                ...
                <div class="col-md-2" style="padding-top:6px;"><select name="type" id="type" class="form-control "  ><option value="0">Mode</option>
                     <option value="0">View All Modes</option>
				<?php
					...
					<?php }	}else{?>
					<option value="0">No Modes</option>
					<?php } ?>
	...
	if(!$getTableMemIds[0]==0){
		...
		*/
		...
		if($paymentMode==0){
			...
			$addInfo=getBankTransactionDetailsByMemId($mids);
		}
