...
if(isset($_GET['did'])&&$_GET['did']!=''){
	...
	$selimgQry=mysql_fetch_row(mysql_query("select * from  `photos` where `id`='$delid'"));
	$oldimg=$selimgQry[1];

	$unlinkimg=unlink("../photos/".$oldimg);
	
		$delQry=mysql_query("delete from `photos` where id='$delid'");
		if($delQry){
			$successflag=1;
		}else{
			$successflag=0;
	...
	if($successflag=1){
		header("location:photos.php?msg=dls");
	}else{
		header("location:photos.php?msg=dlf");
	} 
 }
...
if(isset($_POST['submit'])|| isset($_POST['submit_x'])){
	...
	$nwsimg=$_FILES['image1']['name'];
	$newsimagename=basename($_FILES['image1']['name']);
	$newsimagenamesrc=$_FILES['image1']['tmp_name'];
	...
	$sqlQry="insert into `ppac_photos` set `image`='$imgName' , `status`='1',`name`='$name',`date`='$date',`category_id`='$category_id'  ";
	...
	if($successflag==1){
		header("Location:photos.php?msg=ins&id=$category_id");
	}
	if($successflag==0){
		header("Location:photos.php?msg=inf&id=$category_id");
	}
...
if(isset($_POST['update'])){
	...
	$cid=$hidCid;
$photos = $_POST['operational'];
	if(count($photos)>0){
		$operational=implode(",",$photos);	
	}else{
		$operational=0;
	}
 	 $title=mysql_real_escape_string($title);
	
			$resImgQry=mysql_fetch_row(mysql_query("select `imagepath` from `photos` where `id` = '$id'"));
			$oldimg=$resImgQry[0];
			...
			if($img!=''){
			 ...
			 $moveimg=move_uploaded_file($imagenamesrc,'../photos/'.$imgName);
					if($moveimg){
						...
						$sqlImgQry=mysql_query("Update `photos` set `imagepath`='$imgName' ,`title`='$title',`position`='$position',`content`='$content'  ,`operational`='$operational' where `id`='$id'");
					}else{
						$successflag=0;
					}
			}else{
			  $sqlImgQry=mysql_query("Update `photos` set `title`='$title' ,`position`='$position',`content`='$content' ,`operational`='$operational'   where `id`='$id'");
			  if($sqlImgQry){
			   $successflag=1;
			  }else{
			   $successflag=0;
	...
	if($successflag==1){
	    header("Location:photos.php?msg=ups");
	}else{
        header("Location:photos.php?msg=upf");
	}
...
<body class="theme-dark">
	...
	<div id="container">
					...
					<div class="col-md-12">
						<div class="widget box">
							<div class="widget-header">                            
								...
								<h4><i class="icon-reorder"></i> Add Photos </h4>
							</div>
		...
		<?php echo $msg; ?>
             ...
             <?php } ?>
              ...
              <tr>
				...
				<?php if(isset($_GET['eid'])&&$_GET['eid']!=''){ 
					...
					$execQry=mysql_fetch_row(mysql_query("select * from `photos` where `id`='$eid'"));
              ...
              <td colspan="2" align="left">
				...
				<tr >
				<td>&nbsp;</td>
				<td colspan="2" align="left"><input type="hidden" name="hidId" value="<?php echo $eid; ?>"><input type="hidden" name="updateId" id="updateId"><input type="submit" name="update" class="btn" value=" Update Gallery">&nbsp;<input type="button" name="cancel" value=" Cancel " onClick="javascript:window.location.href='photos.php'" class="btn"></td>
				</tr>
            ...
            <tr >
            <td align="left" class="blackbold">Image heading </td>
            <td colspan="2"><input type="text"  class="form-control " style="letter-spacing:1px;" name="name" id="title" ></td>
            </tr>
				
            <tr >
              <td>Add Image</td>
              <td align="left"><input  class="filetype" type="file" name="image1" ></td>
              <td align="left" style="font-size:12px;">Image ration should be 1:1 eg 200*200 </td>
            </tr>
                         
            <tr >
            <td>&nbsp;</td>
            <td colspan="2" align="left"><input type="hidden" name="category_id" value="<?php echo $category_id ?>" >
<input type="submit" name="submit" class="btn" value=" Add Image ">&nbsp;
<input type="button" name="cancel" value=" Cancel " onClick="javascript:window.location.href='photos_category.php'" class="btn">
</td>
            </tr>
				<?php } ?>
  				</form>
...
$j(document).ready(function(){ 
	...
	$j(function() {
		$("tbody").sortable({ opacity: 0.6, cursor: 'move', update: function() {
			var order = $(this).sortable("serialize") + '&action=updateRecordsListings&type=photos'; 
  ...
  <?php 
  	$sqlQry=mysql_query("select * from `ppac_photos` where `category_id`='$category_id' order by `id` desc ");
	$i=0;
  ...
  ?>
  <tr bgcolor="#FFFFFF" id="recordsArray_<?php echo $fetch['id']; ?>">
    ...
    <td align="center" >
      <table    border="0" cellpadding="0" cellspacing="0" style="padding:0px">
        <tr>
          <td align="right" ><a href="photos.php?did=<?php echo base64_encode($fetch['id']) ?>&id=<?php echo $category_id; ?>"  class="confirm-dialog" onClick="return confirm(' Are you sure you want to delete !!')"><button class="btn"><i class="icon-trash"></i></button></a></td>
         <!-- <td align="center" ><a href="photos.php?eid=<?php echo base64_encode($fetch['id']) ?>"><button class="btn"><i class="icon-edit"></i></button>	</a></td>
          <td align="left"  ><input class='uniform' type="checkbox" id="check<?php echo $fetch['id']  ?>" value="<?php echo $fetch['status']  ?>" onClick="updateStatus('<?php echo $fetch['id'];  ?>','photos',2)" <?php if($fetch['status']==1){echo 'checked';} ?>></td>
          -->
