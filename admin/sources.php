...
$delQry=mysql_query("delete from `sources` where `id`='$did'");
	if($delQry){
		header("location:sources.php?msg=dls");
	}else{
		header("location:sources.php?msg=dlf");
	}
}
...
$sqlQry="insert into `sources` set `name`='$name',`status`='1'";
$execQry=mysql_query($sqlQry);
if($execQry){
	header("location:sources.php?msg=ins");
}else{
	header("location:sources.php?msg=inf");
}
...
$name=mysql_real_escape_string($_POST['name']);
$id=$_POST['hidval'];
$sqlQry="Update `sources` set `name`='$name' where `id`='$id' ";
$execQry=mysql_query($sqlQry);
if($execQry){
	header("location:sources.php?msg=ups");
}else{
	header("location:sources.php?msg=upf");
}
...
<body class="theme-dark">
					...
					<div class="col-md-12">
						<div class="widget box">
							<div class="widget-header">
								<h4><i class="icon-reorder"></i> Add sources</h4>
							</div>
             ...
             <?php } ?>
              ...
              <tr>
				...
				<?php if(isset($_GET['eid'])&&$_GET['eid']!=''){ 
					...
					$execQry=mysql_fetch_row(mysql_query("select * from `sources` where `id`='$eid'"));
					$name=$execQry[1];
				...
				<tr >
				<td width="17%" align="left" class="blackbold">Update  sources Name</td>
				...
				<tr >
				<td>&nbsp;</td>
				<td colspan="2" align="left"><input type="submit" name="update" class="btn" value=" Update sources	 ">&nbsp;<input type="button" name="cancel" value=" Cancel " onClick="javascript:window.location.href='sources.php'" class="btn"></td>
				</tr>
				
				<?php }else{?>
				<tr >
				<td align="left" class="blackbold">Add sources  Name</td>
				...
				<tr >
				<td>&nbsp;</td>
				<td colspan="2" align="left"><input type="submit" name="submit" class="btn" value=" Add sources  ">&nbsp;<input type="button" name="cancel" value=" Cancel " onClick="javascript:window.location.href='sources.php'" class="btn"></td>
				</tr>
				<?php } ?>
  				</form>
</table>
			  ...
			  <tr><td colspan="2">
                ...
                <div class="row">
					<div class="col-md-12">
						<div class="widget box">
							<div class="widget-header">
								<h4><i class="icon-reorder"></i> sources</h4>
  ...
  <?php 
  	$sqlQry=mysql_query("select * from `sources` order by `id`");
	$i=0;
  ...
  <tr bgcolor="#FFFFFF">
	...
	<tr >
		<td align="right" ><a href="sources.php?did=<?php echo base64_encode($fetch['id']) ?>"  class="confirm-dialog" onClick="return confirm(' Are you sure you want to delete !!')"><button class="btn"><i class="icon-trash"></i></button></a></td>
		<td align="center" ><a href="sources.php?eid=<?php echo base64_encode($fetch['id']) ?>"><button class="btn"><i class="icon-edit"></i></button>	</a></td>
        
        
		<td align="left"  ><input class='uniform' type="checkbox" id="check<?php echo $fetch['id']  ?>" value="<?php echo $fetch['status']  ?>" onClick="updateStatus('<?php echo $fetch['id'];  ?>','sources',2)" <?php if($fetch['status']==1){echo 'checked';} ?>></td>
