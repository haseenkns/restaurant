...
$select1=mysql_num_rows(mysql_query("select * from cms where  `key`='15'"));
			//$select=mysql_num_rows(mysql_query("select * from cms where  `key`='2'"));
		   if($select1<1)
		   {   
		  	    $insert=mysql_query("insert into cms set `id`='',`content`='$content',`title`='".($_POST["heading"])."' ,`key`='2' ");
		   		if($insert){
					header("location:guidlines.php?msg=ins");
				}else{
					header("location:guidlines.php?msg=inf");
				}
		   }
		   else
		   {
			   $update=mysql_query("update cms  set content='$content',`title`='".($_POST["heading"])."'  where `key`='15'");
			   if($update){
			   		header("location:guidlines.php?msg=ups");
			   }else{
			   		header("location:guidlines.php?msg=upf");
			   }
			}
		}

	$myselect=mysql_query("select * from cms where  `key`='15'");
					...
					<div class="col-md-12">
						<div class="widget box">
							<div class="widget-header">
								<h4><i class="icon-reorder"></i> Guidlines Content</h4>
							</div>
                        ...
                        <form action="" method="post" name="addfrm" id="addfrm" onSubmit="return validate();">
                          <table width="100%" border="0" cellpadding="4" cellspacing="1" >
                            <tr>
                              ...
                              <textarea type="text" name="heading" class="form-control wysiwyg"  rows="5" cols="72"><?php echo $pageheading ;?></textarea>
